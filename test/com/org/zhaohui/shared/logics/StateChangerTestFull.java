...
import static com.org.zhaohui.shared.basics.PieceKind.BISHOP;
import static com.org.zhaohui.shared.basics.PieceKind.KNIGHT;
import static com.org.zhaohui.shared.basics.PieceKind.ROOK;
import static com.org.zhaohui.shared.basics.State.COLS;
import static com.org.zhaohui.shared.basics.State.ROWS;
...
public class StateChangerTestFull extends StateChangerTest {
  ...
  public void testMovingAfterFiftyMoveRule() {
    ...
    stateChanger.makeMove(start, new Move(new Position(7, 1),
  ...
  public void testMovingAfterCheckmate() {
    ...
    start = new State(Color.BLACK, board, new boolean[] { true, true },
        new boolean[] { true, true }, null, 0, new GameResult(Color.WHITE,
            GameResultReason.CHECKMATE));
    stateChanger.makeMove(start, new Move(new Position(7, 4),
  ...
  public void testIllegalCastlingWhenRookHasBeenMoved() {
    ...
    stateChanger.makeMove(start, new Move(new Position(7, 7),
        new Position(6, 7), null));
    stateChanger.makeMove(start, new Move(new Position(0, 4),
        new Position(1, 4), null));
    stateChanger.makeMove(start, new Move(new Position(6, 7),
        new Position(7, 7), null));
    stateChanger.makeMove(start, new Move(new Position(1, 4),
  ...
  public void testIllegalCastlingWhenKingHasBeenMoved() {
    ...
    stateChanger.makeMove(start, new Move(new Position(0, 4),
        new Position(1, 4), null));
    stateChanger.makeMove(start, new Move(new Position(6, 4),
        new Position(7, 4), null));
    stateChanger.makeMove(start, new Move(new Position(1, 4),
        new Position(0, 4), null));
  ...
  public void testIllegalPawnMovingTwoSquaresForwardAfterLeavingStartPosition() {
    start.setTurn(Color.BLACK);
    stateChanger.makeMove(start, new Move(new Position(7, 0),
        new Position(6, 0), null));
    stateChanger.makeMove(start, new Move(new Position(1, 0),
        new Position(2, 0), null));
    stateChanger.makeMove(start, new Move(new Position(6, 0),
  ...
  public void testIllegalPromotionToKing() {
    ...
    start = new State(Color.BLACK, board, new boolean[] { true, true },
        new boolean[] { true, true }, null, 0, null);
    Move move = new Move(new Position(1, 7), new Position(0, 7), PieceKind.KING);
    stateChanger.makeMove(start, move);
  ...
  public void TestWhiteKingMoveUp() {
    ...
    expected
        .setNumberOfMovesWithoutCaptureNorPawnMoved(CurNumberOfMovesWithoutCaptureNorPawnMoved + 1);
    stateChanger.makeMove(start, move);
    assertEquals(expected, start);
  }
  ...
  /**
   * Tests for Legal moves: A powerset of 3 basic test scenarios: 1. Queen/King
   * Side 2. Sparse/Dense Rank - Presence of other pieces on the same rank 3.
   * Other rook moved/not moved
   */
  ...
  public void testLegalCastleKingSideRandomPawnMoves() {
    ...
    // squares ahead
    start.setEnpassantPosition(new Position(4, 1)); // Set its enpassant
                                                    // position
    Move move = new Move(new Position(7, 4), new Position(7, 6), null);
    State expected = start.copy(); // Copy the current (start) state
    setExpectedStateAfterKingSideCastle(expected);
  ...
  void initforsanjana() {
    ...
    // State copied = new State(Color.WHITE, board, new boolean[]{true, true},
    // new boolean[]{true, true}, null, 0, null);
    start = new State();
  ...
  // If white bishop tries to capture a black queen but there's another piece in
  // the path
  ...
  // If white knight tries to capture the black bishop this will expose the
  // white king
  // to check from black rook, making this an illegal capture but still a legal
  // move as in
  ...
  // A king tries to capture a black piece, which is on a square that is
  // attacked by a piece of the opponent;
  // regardless whether this piece is `pinned' (would cause check to its own
  // king) when moved or not.
  // Such a move would still be regarded as moving the king into check. hence
  // illegal capture
  ...
  public void testIllegalCaptureKingExposedTOCheck() {
    ...
    // if king tries to capture the bishop it could expose itself to check from
    // black rook
    // even though the rook is pinned to its' king. It will still be an illegal
    // capture
  ...
  // If white king, black knight and black king are in one row next to each
  // other;
  // and the white king tries to capture the knight standing next to its' black
  // king
  ...
  // If white king tries to capture a black piece exposing itself to check i.e.
  // an illegal capture
  @Test(expected = IllegalMove.class)
  public void testKingMovesInCheckIllegalCapture() {
    ...
    // If king captures bishop it exposes itself to check, making this an
    // illegal capture
    Move move = new Move(new Position(1, 4), new Position(2, 5), null);
    stateChanger.makeMove(start, move);
  }

  // If white knight tries to capture a piece by an illegal move i.e. an illegal
  // capture
  @Test(expected = IllegalMove.class)
  ...
  // If white pawn tries to capture a black piece in forward direction i.e an
  // illegal capture
  @Test(expected = IllegalMove.class)
  ...
  // If a white pawn captures a black piece legally but stands in-front of
  // another
  // white pawn. It is still a legal capture and is called doubling or doubled
  // pawns.
  @Test()
  ...
  // If white Queen tries to capture a piece when it is not her turn i.e an
  // illegal capture
  @Test(expected = IllegalMove.class)
  ...
  // If white rook tries to capture a black knight but there's another piece in
  // the path
  // i.e an illegal capture
  @Test(expected = IllegalMove.class)
  ...
  public void testPawnCaptureBishop() {
    ...
    State original = new State(BLACK, board, CastleBool, CastleBool, null, 7,
        null);
  ...
  public void testPawnCaptureRook() {
    ...
    State original = new State(BLACK, board, CastleBool, CastleBool, null, 3,
        null);
  ...
  public void testPawnCaptureKnight() {
    ...
    State original = new State(BLACK, board, CastleBool, CastleBool, null, 0,
        null);
  ...
  public void testQueenCapturePawn() {
    ...
    Piece[][] board = new Piece[ROWS][COLS];
    Move move = new Move(new Position(4, 4), new Position(3, 3), null);
    State original = new State(BLACK, board, CastleBool, CastleBool, null, 2,
        null);
  ...
  public void testQueenCaptureBishop() {
    ...
    State original = new State(BLACK, board, CastleBool, CastleBool, null, 5,
        null);
  ...
  public void testRookCaptureBishop() {
    ...
    State original = new State(BLACK, board, CastleBool, CastleBool, null, 8,
        null);
  ...
  public void TestTryToEscapeFromKnightBlackByCastlingQueenSide() {
     ...
     * escaped.setPiece(0, 2, new Piece(WHITE, KING)); escaped.setPiece(0, 0,
     * null); escaped.setPiece(0, 3, new Piece(WHITE, ROOK));
  ...
  public void TestTryToEscapeFromBishopBlackByCastlingKingSide() {
    ...
    /*
     * State escaped = checked.copy(); escaped.setPiece(0, 4, null);
     * escaped.setPiece(0, 6, new Piece(WHITE, KING)); escaped.setPiece(0, 7,
     * null); escaped.setPiece(0, 5, new Piece(WHITE, ROOK));
     * escaped.setTurn(BLACK); escaped.setCanCastleKingSide(false);
     * escaped.setCanCastleQueenSide(false);
     * escaped.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
     */
  ...
  public void TestTryToEscapeFromQueenBlackByMoveAPiceInTheCheckWayButEndangerByOther() {
     ...
     * escaped.setPiece(3, 1, new Piece(WHITE, KNIGHT)); escaped.setTurn(BLACK);
     * escaped.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
     */
  ...
  public void testPromotionToRookEmptyPiece() {
    Move move = new Move(new Position(6, 3), new Position(7, 3), PieceKind.ROOK);
    start.setPiece(7, 3, null);
    start.setPiece(6, 3, null);
    start.setPiece(1, 3, null);
    start.setPiece(6, 4, null);
  ...
  public void testPromotionToRookWithCapture() {
    Move move = new Move(new Position(6, 7), new Position(7, 6), PieceKind.ROOK);
  ...
  public void testIllegalPromotionNotPromote1() {
    Move move = new Move(new Position(6, 5), new Position(7, 5), null);
    start.setPiece(6, 5, null);
    start.setPiece(1, 5, null);
    start.setPiece(6, 5, new Piece(Color.WHITE, PieceKind.PAWN));
  ...
  public void testIllegalPromotionWrongPromotionKind1() {
    Move move = new Move(new Position(6, 1), new Position(7, 1), PieceKind.PAWN);
    start.setPiece(6, 1, null);
    start.setPiece(1, 1, null);
    start.setPiece(6, 1, new Piece(Color.WHITE, PieceKind.PAWN));
  ...
  public void testIllegalPromotionWrongPromotionKind2() {
    Move move = new Move(new Position(6, 5), new Position(7, 5), PieceKind.KING);
    start.setPiece(6, 5, null);
    start.setPiece(1, 5, null);
    start.setPiece(6, 5, new Piece(Color.WHITE, PieceKind.PAWN));
  ...
  public void testIllegalPromotionPromoteOtherPiece2() {
    Move move = new Move(new Position(6, 1), new Position(7, 1), PieceKind.ROOK);
    start.setPiece(6, 1, null);
    start.setPiece(1, 1, null);
  ...
  /*
   * Test castle when canCastleKingside and canCastleQueenside are both set true
   * for white
   */
  @Test
  ...
  public void testPieceKindWhenMoveBlackPawnTwoSquares() {
    ...
    Move move = new Move(new Position(6, 1), new Position(4, 1), null);
    stateChanger.makeMove(start, move);
    assertEquals(PAWN, start.getPiece(start.getEnpassantPosition()).getKind());
  }
  ...
  public void testColorWhenMoveBlackPawnTwoSquares() {
    ...
    stateChanger.makeMove(start, move);
    assertEquals(BLACK, start.getPiece(start.getEnpassantPosition()).getColor());
  }
  ...
  public void testOtherMovementsWontAffectEnpassantPosition_Bishop() {
    ...
    start.setTurn(BLACK);
    stateChanger.makeMove(start, new Move(new Position(7, 2),
        new Position(6, 1), null));
    assertEquals(null, start.getEnpassantPosition());
  }
  ...
  public void testOtherMovementsWontAffectEnpassantPosition_Queen() {
    ...
    start.setTurn(BLACK);
    stateChanger.makeMove(start, new Move(new Position(7, 3),
  ...
  /*
   * @Test public void testBlackKnightLegalMovement() { //create an initial
   * board with several Pieces State before = new State(BLACK, new Piece[8][8],
   * new boolean[]{false, false}, new boolean[]{false, false}, null,0, null);
   * 
   * before.setPiece(0, 4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   * Piece(BLACK,KING)); before.setPiece(4,3, new Piece(BLACK,KNIGHT));
   * before.setPiece(3, 3, new Piece(WHITE,PAWN)); before.setPiece(3, 4, new
   * Piece(BLACK,PAWN)); before.setPiece(4,4,new Piece(WHITE,PAWN));
   * 
   * Move move = new Move(new Position(4,3),new Position(2,4),null);
   * 
   * //change the position of several Pieces to make it as expected State
   * before1 = before.copy(); State expected1 = before1.copy();
   * 
   * expected1.setTurn(WHITE); expected1.setPiece(4, 3, null);
   * expected1.setPiece(2,4,new Piece(BLACK,KNIGHT));
   * expected1.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
   * stateChanger.makeMove(before1, move); assertEquals(before1,expected1);
   * 
   * //test for movement of black knight to position occupied by one white pawn
   * State before2 = before.copy(); before2.setPiece(3, 3, null);
   * before2.setPiece(2,4,new Piece(WHITE,PAWN)); State expected2 =
   * before2.copy();
   * 
   * expected2.setTurn(WHITE); expected2.setPiece(4, 3, null);
   * expected2.setPiece(2,4,new Piece(BLACK,KNIGHT));
   * expected2.setNumberOfMovesWithoutCaptureNorPawnMoved(0);
   * stateChanger.makeMove(before2, move); assertEquals(before2,expected2);
   * 
   * //test for movement of black knight to unoccupied position State before3 =
   * before.copy(); State expected3 = before3.copy(); Move move2 = new Move(new
   * Position(4,3),new Position(3,1),null); expected3.setTurn(WHITE);
   * expected3.setPiece(4, 3, null); expected3.setPiece(3,1,new
   * Piece(BLACK,KNIGHT));
   * expected3.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
   * stateChanger.makeMove(before3, move2); assertEquals(before3,expected3);
   * 
   * }
   * 
   * 
   * //test for illegal movement of black knight to position occupied with same
   * color piece
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackKnightMoveToBlackPiece() { Move move = new Move(new
   * Position(4,3),new Position(2,4),null); State before = new State(BLACK, new
   * Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null);
   * 
   * before.setPiece(0, 4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   * Piece(BLACK,KING)); before.setPiece(4,3, new Piece(BLACK,KNIGHT));
   * before.setPiece(3, 3, new Piece(WHITE,PAWN)); before.setPiece(2, 4, new
   * Piece(BLACK,PAWN)); before.setPiece(4,4,new Piece(WHITE,PAWN));
   * 
   * 
   ...
   * 
   * @Test(expected = IllegalMove.class) public void testBlackKnightMoveSquare()
   ...
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackKnightMoveDiagonal() { State before = new State(BLACK, new
   * Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null);
   * 
   * before.setPiece(0, 4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   * Piece(BLACK,KING)); before.setPiece(4,3, new Piece(BLACK,KNIGHT));
   * before.setPiece(3, 3, new Piece(WHITE,PAWN)); before.setPiece(3, 4, new
   * Piece(BLACK,PAWN)); before.setPiece(4,4,new Piece(WHITE,PAWN)); Move move =
   * new Move(new Position(4,3),new Position(2,1),null);
   * 
   * stateChanger.makeMove(before, move); }
   * 
   * 
   ...
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackKnightMoveVertical() { State before = new State(BLACK, new
   * Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null);
   * 
   * before.setPiece(0, 4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   ...
   * before.setPiece(3, 3, new Piece(WHITE,PAWN)); before.setPiece(3, 4, new
   * Piece(BLACK,PAWN)); before.setPiece(4,4,new Piece(WHITE,PAWN)); Move move =
   * new Move(new Position(4,3),new Position(6,3),null);
   * 
   * stateChanger.makeMove(before, move); }
   * 
   ...
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackKnightMoveHorizontal() { State before = new State(BLACK, new
   * Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null);
   * 
   * before.setPiece(0, 4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   ...
   * before.setPiece(3, 3, new Piece(WHITE,PAWN)); before.setPiece(3, 4, new
   * Piece(BLACK,PAWN)); before.setPiece(4,4,new Piece(WHITE,PAWN)); Move move =
   * new Move(new Position(4,3),new Position(4,0),null);
   * 
   * stateChanger.makeMove(before, move); }
   * 
   * 
   ...
   * 
   * @Test public void testBlackBishopLegalMove() { //setup the state board
   * State before = new State(BLACK, new Piece[8][8], new boolean[]{false,
   * false}, new boolean[]{false, false}, null,0, null);
   * 
   * before.setPiece(0,4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   * Piece(BLACK,KING)); before.setPiece(4,3, new Piece(BLACK,BISHOP));
   * before.setPiece(2,1, new Piece(WHITE,PAWN)); before.setPiece(7,6, new
   * Piece(BLACK,KNIGHT));
   * 
   * //test for movement of black bishop to unoccupied position State before1 =
   * before.copy(); State expected1 = before1.copy(); Move move1 = new Move(new
   * Position(4,3),new Position(6,1),null);
   * 
   * 
   * expected1.setTurn(WHITE); expected1.setPiece(4,3, null);
   * expected1.setPiece(6,1,new Piece(BLACK,BISHOP));
   * expected1.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
   * stateChanger.makeMove(before1, move1); assertEquals(before1,expected1);
   * 
   * //test for movement of black bishop to occupied position by white piece
   * State before2 = before.copy(); State expected2 = before2.copy(); Move move2
   * = new Move(new Position(4,3),new Position(2,1),null);
   * 
   * expected2.setTurn(WHITE); expected2.setPiece(4,3, null);
   * expected2.setPiece(2,1,new Piece(BLACK,BISHOP));
   * expected2.setNumberOfMovesWithoutCaptureNorPawnMoved(0);
   * stateChanger.makeMove(before2, move2); assertEquals(before2,expected2);
   * 
   * //test for movement of black biship to unoccupied position State before3 =
   * before.copy(); State expected3 = before3.copy(); Move move3 = new Move(new
   * Position(4,3),new Position(6,5),null);
   * 
   * expected3.setTurn(WHITE); expected3.setPiece(4,3, null);
   * expected3.setPiece(6,5,new Piece(BLACK,BISHOP));
   * expected3.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
   * stateChanger.makeMove(before3, move3); assertEquals(before3,expected3); }
   * 
   * //test for movement of black bishop to vertical position
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackBishopMoveVertical(){ Move move = new Move(new Position(4,3),new
   * Position(2,3),null); State before = new State(BLACK, new Piece[8][8], new
   * boolean[]{false, false}, new boolean[]{false, false}, null,0, null);
   * 
   * before.setPiece(0,4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   * Piece(BLACK,KING)); before.setPiece(4,3, new Piece(BLACK,BISHOP));
   * before.setPiece(2,1, new Piece(WHITE,PAWN)); before.setPiece(7,6, new
   ...
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackBishopMoveHorizontal(){ Move move = new Move(new Position(4,3),new
   * Position(4,6),null); State before = new State(BLACK, new Piece[8][8], new
   * boolean[]{false, false}, new boolean[]{false, false}, null,0, null);
   * 
   * before.setPiece(0,4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   * Piece(BLACK,KING)); before.setPiece(4,3, new Piece(BLACK,BISHOP));
   ...
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackBishopMoveNotDiagonal(){ Move move = new Move(new
   * Position(4,3),new Position(6,6),null); State before = new State(BLACK, new
   * Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null);
   * 
   * before.setPiece(0,4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   ...
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackBishopMoveOverPiece(){ Move move = new Move(new Position(4,3),new
   * Position(1,0),null); State before = new State(BLACK, new Piece[8][8], new
   * boolean[]{false, false}, new boolean[]{false, false}, null,0, null);
   * 
   * before.setPiece(0,4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   * Piece(BLACK,KING)); before.setPiece(4,3, new Piece(BLACK,BISHOP));
   ...
   * 
   * 
   * //test for movement of black bishop to position occupied by piece with same
   * color
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackBishopMoveToPositionOfSameColor(){ State before = new State(BLACK,
   * new Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null);
   * 
   * before.setPiece(0,4, new Piece(WHITE,KING)); before.setPiece(7,4, new
   * Piece(BLACK,KING)); before.setPiece(4,3, new Piece(BLACK,BISHOP));
   * before.setPiece(2,1, new Piece(WHITE,PAWN)); before.setPiece(7,6, new
   * Piece(BLACK,KNIGHT)); Move move = new Move(new Position(4,3),new
   * Position(7,6),null);
   * 
   * stateChanger.makeMove(before, move); }
   * 
   ...
   * 
   * @Test public void testBlackRookLegalMove(){ State before = new State(BLACK,
   * new Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null); before.setPiece(0,4,new Piece(WHITE,KING));
   * before.setPiece(7, 4, new Piece(BLACK,KING)); before.setPiece(4,3, new
   * Piece(BLACK,ROOK)); before.setPiece(3, 3, new Piece(BLACK,KNIGHT));
   * before.setPiece(4,6,new Piece(WHITE,PAWN));
   * 
   * 
   * //test for movement of black rook to unoccupied position State before1 =
   * before.copy(); State except1 = before1.copy(); Move move1 = new Move(new
   * Position(4,3),new Position(4,0),null);
   * 
   * stateChanger.makeMove(before1, move1); except1.setPiece(4, 3, null);
   * except1.setPiece(4, 0, new Piece(BLACK,ROOK)); except1.setTurn(WHITE);
   * except1.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
   * assertEquals(except1,before1);
   * 
   * //test for movement of black rook to position occupied by op piece State
   * before2 = before.copy(); State except2 = before2.copy(); Move move2 = new
   * Move(new Position(4,3),new Position(4,6),null);
   * 
   * stateChanger.makeMove(before2, move2); except2.setPiece(4, 3, null);
   * except2.setPiece(4, 6, new Piece(BLACK,ROOK)); except2.setTurn(WHITE);
   ...
   * }
   * 
   * //test for movement of black rook to position occupied by piece with same
   * color
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackRookMoveToPositionOfSameColor(){ State before = new State(BLACK,
   * new Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null); before.setPiece(0,4,new Piece(WHITE,KING));
   * before.setPiece(7, 4, new Piece(BLACK,KING)); before.setPiece(4,3, new
   * Piece(BLACK,ROOK)); before.setPiece(3, 3, new Piece(BLACK,KNIGHT));
   * before.setPiece(4,6,new Piece(WHITE,PAWN)); Move move = new Move(new
   * Position(4,3),new Position(3,3),null);
   * 
   * stateChanger.makeMove(before, move); }
   * 
   * //test for movement of black rook to non-vertical and non-horizontal
   * position
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackRookNotMoveVerticalNorHorizontal(){ State before = new
   * State(BLACK, new Piece[8][8], new boolean[]{false, false}, new
   * boolean[]{false, false}, null,0, null); before.setPiece(0,4,new
   * Piece(WHITE,KING)); before.setPiece(7, 4, new Piece(BLACK,KING));
   * before.setPiece(4,3, new Piece(BLACK,ROOK)); before.setPiece(3, 3, new
   * Piece(BLACK,KNIGHT)); before.setPiece(4,6,new Piece(WHITE,PAWN)); Move move
   * = new Move(new Position(4,3),new Position(2,2),null);
   * 
   * 
   * stateChanger.makeMove(before, move); }
   * 
   ...
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackRookMoveOverOpPiece(){ State before = new State(BLACK, new
   * Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null); before.setPiece(0,4,new Piece(WHITE,KING));
   * before.setPiece(7, 4, new Piece(BLACK,KING)); before.setPiece(4,3, new
   * Piece(BLACK,ROOK)); before.setPiece(3, 3, new Piece(BLACK,KNIGHT));
   * before.setPiece(4,6,new Piece(WHITE,PAWN)); Move move = new Move(new
   * Position(4,3),new Position(4,7),null);
   * 
   * stateChanger.makeMove(before, move); }
   * 
   ...
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackRookMoveOverSamePiece(){ State before = new State(BLACK, new
   * Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null); before.setPiece(0,4,new Piece(WHITE,KING));
   * before.setPiece(7, 4, new Piece(BLACK,KING)); before.setPiece(4,3, new
   * Piece(BLACK,ROOK)); before.setPiece(3, 3, new Piece(BLACK,KNIGHT));
   * before.setPiece(4,6,new Piece(WHITE,PAWN)); Move move = new Move(new
   * Position(4,3),new Position(1,3),null);
   * 
   * 
   * stateChanger.makeMove(before, move); }
   * 
   ...
   * 
   * @Test public void testBlackQueenLegalMove(){ State before = new
   * State(BLACK, new Piece[8][8], new boolean[]{false, false}, new
   * boolean[]{false, false}, null,0, null);
   * 
   * before.setPiece(0, 4, new Piece(WHITE,KING)); before.setPiece(7, 4,new
   * Piece(BLACK,KING)); before.setPiece(3, 3,new Piece(BLACK,QUEEN));
   * before.setPiece(5, 1,new Piece(BLACK,PAWN)); before.setPiece(3, 7,new
   * Piece(WHITE,PAWN)); //test for movement of black queen to unoccupied
   * vertical position State before1 = before.copy(); State expected1 =
   * before1.copy(); Move move1 = new Move(new Position(3,3),new
   * Position(6,3),null);
   * 
   * expected1.setPiece(3, 3,null); expected1.setPiece(6, 3, new
   * Piece(BLACK,QUEEN)); expected1.setTurn(WHITE);
   * expected1.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
   * stateChanger.makeMove(before1, move1); assertEquals(before1,expected1);
   * 
   * 
   * //test for movement of black queen to unoccupied horizontal position State
   * before2 = before.copy(); State expected2 = before2.copy(); Move move2 = new
   * Move(new Position(3,3),new Position(3,0),null);
   * 
   * expected2.setPiece(3, 3,null); expected2.setPiece(3, 0, new
   * Piece(BLACK,QUEEN)); expected2.setTurn(WHITE);
   * expected2.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
   * stateChanger.makeMove(before2, move2); assertEquals(before2,expected2);
   * 
   * 
   * //test for movement of black queen to unoccupied diagonal position State
   * before3 = before.copy(); State expected3 = before3.copy(); Move move3 = new
   * Move(new Position(3,3),new Position(4,2),null);
   * 
   * expected3.setPiece(3, 3,null); expected3.setPiece(4, 2, new
   * Piece(BLACK,QUEEN)); expected3.setTurn(WHITE);
   * expected3.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
   * stateChanger.makeMove(before3, move3); assertEquals(before3,expected3);
   * 
   * 
   * //test for movement of black queen to unoccupied diagonal position State
   * before4 = before.copy(); State expected4 = before4.copy(); Move move4 = new
   * Move(new Position(3,3),new Position(1,1),null);
   * 
   * expected4.setPiece(3, 3,null); expected4.setPiece(1, 1, new
   * Piece(BLACK,QUEEN)); expected4.setTurn(WHITE);
   * expected4.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
   * stateChanger.makeMove(before4, move4); assertEquals(before4,expected4);
   * 
   * 
   * //test for movement of black queen to occupied position with op piece State
   * before5 = before.copy(); State expected5 = before5.copy(); Move move5 = new
   * Move(new Position(3,3),new Position(3,7),null);
   * 
   * expected5.setPiece(3, 3,null); expected5.setPiece(3, 7, new
   * Piece(BLACK,QUEEN)); expected5.setTurn(WHITE);
   * expected5.setNumberOfMovesWithoutCaptureNorPawnMoved(0);
   * stateChanger.makeMove(before5, move5); assertEquals(before5,expected5); }
   * 
   * 
   * //test for movement of black queen over piece
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackQueenMoveOverPiece(){ State before = new State(BLACK, new
   * Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null);
   * 
   * before.setPiece(0, 4, new Piece(WHITE,KING)); before.setPiece(7, 4,new
   * Piece(BLACK,KING)); before.setPiece(3, 3,new Piece(BLACK,QUEEN));
   * before.setPiece(5, 1,new Piece(BLACK,PAWN)); before.setPiece(3, 7,new
   * Piece(WHITE,PAWN)); Move move = new Move(new Position(3,3),new
   * Position(6,0),null);
   * 
   * stateChanger.makeMove(before, move); }
   * 
   ...
   * 
   * @Test(expected = IllegalMove.class) public void
   * testBlackQueenMoveWrongDirection(){ State before = new State(BLACK, new
   * Piece[8][8], new boolean[]{false, false}, new boolean[]{false, false},
   * null,0, null);
   * 
   * before.setPiece(0, 4, new Piece(WHITE,KING)); before.setPiece(7, 4,new
  ...
  public void init() {
    ...
    start = new State(BLACK, board, new boolean[2], new boolean[2], null, 0,
        null);
  }
  ...
  public void setup(Position blackOldPosition, Position blackNewPosition,
      Position whiteOldPosition, Position whiteNewPosition) {
  ...
  public void captureEnpassant(Position blackPosition, Position whitePosition,
      Position whitePositionAfterCapture) {
  ...
  public void testWhiteA5CaptureBlackB5() {
    ...
    setup(blackOldPosition, blackNewPosition, whiteOldPosition,
        whiteNewPosition);

    Position whitePositionAfterCaptureEnpassant = new Position(5, 1); //
    captureEnpassant(blackNewPosition, whiteNewPosition,
        whitePositionAfterCaptureEnpassant);

    Move move = new Move(whiteNewPosition, whitePositionAfterCaptureEnpassant,
        null);
  ...
  public void setupInitialState() {
    ...
    for (int i = 1; i < State.COLS; i++) {
      initialState.setPiece(1, i, null); // Set all white pawns null except the
                                         // first one.
    }
  ...
  public void testCannotCaptureEnpassantIfWillExploseKing() {
    ...
    board[0][4] = new Piece(WHITE, KING); // Move pawn at 4,4 will explose white
                                          // king at 0,4 to black queen at 7,4
    State newState = new State(WHITE, board, new boolean[] { false, false },
        new boolean[] { false, false }, new Position(4, 5), 0, null);
  ...
  public void testCannotCaptureEnpassantIfIsUnderCheck() {
    ...
    board[4][5] = new Piece(BLACK, PAWN);
    board[4][4] = new Piece(WHITE, PAWN);
    board[0][1] = new Piece(WHITE, KING); // White king is under check by queen
                                          // at 7,1
    State newState = new State(WHITE, board, new boolean[] { false, false },
        new boolean[] { false, false }, new Position(4, 5), 0, null);
    Move move = new Move(new Position(4, 4), new Position(5, 5), null);
  ...
  public void testCaptureEnpassantUnderCheck_ByPawn() {
    ...
    board[4][5] = new Piece(BLACK, PAWN);
    board[4][4] = new Piece(WHITE, PAWN);
    board[3][4] = new Piece(WHITE, KING); // White King is under chech by pawn
                                          // at 4,5
    State newState = new State(WHITE, board, new boolean[] { false, false },
        new boolean[] { false, false }, new Position(4, 5), 0, null);
    Move move = new Move(new Position(4, 4), new Position(5, 5), null);
  ...
  public void testCaptureEnpassantUnderCheck_ByQueen() {
    ...
    board[4][5] = new Piece(BLACK, PAWN);
    board[4][4] = new Piece(WHITE, PAWN);
    board[5][6] = new Piece(WHITE, KING); // White king is under check by queen
                                          // at 5,0
    State newState = new State(WHITE, board, new boolean[] { false, false },
        new boolean[] { false, false }, new Position(4, 5), 0, null);
    Move move = new Move(new Position(4, 4), new Position(5, 5), null);
  ...
  public void testCaptureEnpassantUnderCheck_ByRook() {
    ...
    board[4][5] = new Piece(BLACK, PAWN);
    board[4][4] = new Piece(WHITE, PAWN);
    board[5][6] = new Piece(WHITE, KING); // White king is under check by rook
                                          // at 5,0
    State newState = new State(WHITE, board, new boolean[] { false, false },
        new boolean[] { false, false }, new Position(4, 5), 0, null);
    Move move = new Move(new Position(4, 4), new Position(5, 5), null);
  ...
  public void testBlackKingBeingCheckmateByTwoWhiteRooksAndOneWhiteQueen() {
    ...
    expected.setPiece(new Position(1, 4), new Piece(Color.WHITE,
        PieceKind.QUEEN));
    expected.setNumberOfMovesWithoutCaptureNorPawnMoved(1);
    GameResult result = new GameResult(Color.WHITE, GameResultReason.CHECKMATE);
    expected.setGameResult(result);
  ...
  public void testBlackKingBeingCheckmateByOneWhiteKingAndTwoWhiteBishops() {
    ...
    Move moveWhiteBishop = new Move(new Position(4, 1), new Position(5, 2),
        null);
  ...
  public void testBlackKingBeingCheckmateByOneWhiteKingAndTwoKnights() {
    ...
    Move moveWhiteKnight = new Move(new Position(7, 5), new Position(5, 6),
        null);
  ...
  public void testBlackKingBeingCheckmateByOneWhiteKingAndTwoWhiteRooks() {
    ...
    Move moveWhiteKnight = new Move(new Position(0, 0), new Position(3, 0),
        null);
    stateChanger.makeMove(start, moveWhiteKnight);
    assertEquals(expected, start);
  }
  ...
  public void testBlackKingBeingCheckmateByOneWhiteKingAndOneWhiteQueen() {
    ...
    Move moveWhiteKnight = new Move(new Position(6, 7), new Position(7, 7),
        null);
    stateChanger.makeMove(start, moveWhiteKnight);
    assertEquals(expected, start);
  }
  ...
  public void testMoveOneWhiteBishopAndReachFiftyMoveRule() {
    ...
    Move moveWhiteBishop = new Move(new Position(0, 1), new Position(1, 0),
        null);
    stateChanger.makeMove(start, moveWhiteBishop);
    assertEquals(expected, start);
  }
  ...
  public void MoveOneWhiteKnightAndReachFiftyMoveRule() {
    ...
    Move moveWhiteKnight = new Move(new Position(0, 1), new Position(2, 0),
        null);
    stateChanger.makeMove(start, moveWhiteKnight);
    assertEquals(expected, start);
  }
  ...
  public void testMoveWhiteBishopAndReachStaleMate() {
    ...
    Move moveWhiteBishop = new Move(new Position(4, 2), new Position(5, 3),
        null);
    stateChanger.makeMove(start, moveWhiteBishop);
    assertEquals(expected, start);
  }
  ...
  public void testMoveWhiteKnightAndReachStaleMate() {
    ...
    Move moveWhiteKnight = new Move(new Position(5, 0), new Position(6, 2),
        null);
    stateChanger.makeMove(start, moveWhiteKnight);
    assertEquals(expected, start);
  }
  ...
  public void testPawnCanMoveOneSquare() {
    ...
    expected.setPiece(blackPawnRowSpawnPosition + moveBlackPawnUpOne, 0,
  ...
  public void testIllegalPawnLeapOverOtherPieceMovements() {
    // Trying to move a black pawn for 2 squares while another piece is right in
    // front of it
  ...
  public void testIllegalPromote_NotReach8thRank() {
    ...
    start.setPiece(1, 2, null);
    // start.setPiece(0, 2, null);
    Move move = new Move(new Position(2, 2), new Position(1, 2),
        PieceKind.KNIGHT);
    stateChanger.makeMove(start, move);
  }
  ...
  public void testWhenGameAlreadyOver() {
    start.setNumberOfMovesWithoutCaptureNorPawnMoved(100);
    start.setGameResult(new GameResult(Color.WHITE,
        GameResultReason.FIFTY_MOVE_RULE));
    Move move = new Move(new Position(6, 0), new Position(4, 0), null);
    stateChanger.makeMove(start, move);
  }
  ...
  /*
   * This unit test is not logical since black pawn cannot move over rook. So I
   * deleted it.
   * 
   * @Test //wrong public void
   * testWhitePawnCanEnpassantAlsoCaptureAnotherPiece() { Move move = new
   * Move(new Position(4, 1), new Position(5, 0), null); Piece[][] board = new
   * Piece[8][8]; board[4][0] = new Piece(BLACK, PAWN); board[5][0] = new
   * Piece(BLACK, ROOK); board[4][1] = new Piece(WHITE, PAWN); board[0][3] = new
   * Piece(WHITE, KING); board[7][4] = new Piece(BLACK, KING); State former =
   * new State(WHITE, board, new boolean[] { true, true }, new boolean[] { true,
   * true }, new Position(4, 0), 0, null); State expected = former.copy();
   * expected.setTurn(BLACK); expected.setPiece(4, 0, null);
   * expected.setPiece(5, 0, new Piece(WHITE, PAWN)); expected.setPiece(4, 1,
   * null); expected.setEnpassantPosition(null);
   * expected.setNumberOfMovesWithoutCaptureNorPawnMoved(0);
   * stateChanger.makeMove(former, move); assertEquals(expected, former);
   * 
   * }
   */
  ...
  public void testEnpassantPositionIfGameOver() {
    ...
    Move move = new Move(new Position(6, 1), new Position(7, 1), null);
    State expected = former.copy();
    GameResult gameResult = new GameResult(WHITE, GameResultReason.CHECKMATE);
    expected.setTurn(BLACK);
  ...
  /*
   * @Test //fixed bug for illegal states which is already a checkmate result
   * public void testEnpassantPositionCanNotBeAnywhere() { Piece[][] board = new
   * Piece[8][8]; board[4][0] = new Piece(BLACK, PAWN); board[4][1] = new
   * Piece(WHITE, PAWN); board[0][3] = new Piece(WHITE, KING); board[7][4] = new
   * Piece(BLACK, KING); State former = new State(WHITE, board, new boolean[] {
   * true, true }, new boolean[] { true, true }, new Position(4, 0), 0, null);
  ...
  public void testBlackPawnIllegalPromoteToQueenWithTwoMove() {
    start.setTurn(BLACK);
    Move move = new Move(new Position(6, 0), new Position(4, 0),
        PieceKind.QUEEN);
    stateChanger.makeMove(start, move);
  }
  ...
  public void testBlackPawnIllegalPromoteToQueenAfterCapture() {
    ...
    Move move = new Move(new Position(4, 0), new Position(3, 1),
        PieceKind.QUEEN);
    stateChanger.makeMove(start, move);
  }
  ...
  public void testIllegalMoveKnightWhenGameOver() {
    ...
    gameOverState.setGameResult(new GameResult(Color.BLACK,
        GameResultReason.CHECKMATE));
