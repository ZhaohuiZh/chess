...
public class State {
   ...
   * rook involved are not occupied, the king is not in check, and the king does
  ...
  /**
   * enpassantPosition is either null or the position of a pawn that just moved
   * two squares forward. Note that we must have that: enpassantPosition == null
   * || getPiece(enpassantPosition).getKind()==PieceKind.PAWN
   * 
   ...
   * enemy pawn could have captured it had the same pawn moved only one square
   * forward. http://en.wikipedia.org/wiki/En_passant
   */
  ...
  /**
   * gameResult is null when the game is in progress, and non-null when the game
   * is over.
   */
