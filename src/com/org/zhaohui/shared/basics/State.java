...
public class State {
	...
	/**
	 * canCastleKingSide[0] is true iff the white king has never moved and the
	 * white king-side rook has never moved. (canCastleKingSide[1] is for the
	 * black king and rook) canCastleQueenSide[0] is true iff the white king has
	 * never moved and the white queen-side rook has never moved.
	 * (canCastleQueenSide[1] is for the black king and rook)
	 * 
	 * Castling is a special move in the game of chess involving the king and
	 * either of the original rooks of the same color. It is the only move in
	 * chess (except promotion) in which a player moves two pieces at the same
	 * time. Castling consists of moving the king two squares towards a rook on
	 * the player's first rank, then moving the rook onto the square over which
	 * the king crossed. Castling can only be done if the king has never moved,
	 * the rook involved has never moved, the squares between the king and the
	 * rook involved are not occupied, the king is not in check, and the king
	 * does not cross over or end on a square in which it would be in check.
	...
	/**
	 * enpassantPosition is either null or the position of a pawn that just
	 * moved two squares forward. Note that we must have that: enpassantPosition
	 * == null || getPiece(enpassantPosition).getKind()==PieceKind.PAWN
	 * 
	 ...
	 * an enemy pawn could have captured it had the same pawn moved only one
	 * square forward. http://en.wikipedia.org/wiki/En_passant
	 */
	...
	/**
	 * gameResult is null when the game is in progress, and non-null when the
	 * game is over.
	 */
	...
	public State() {
		for (Color color : Color.values()) {
			for (int c = 0; c < COLS; c++) {
				setPiece(color.isWhite() ? 1 : ROWS - 2, c, new Piece(color,
						PieceKind.PAWN));
			}
	...
	public State(Color turn, Piece[][] board, boolean[] canCastleKingSide,
			boolean[] canCastleQueenSide, Position enpassantPosition,
	...
	public void setNumberOfMovesWithoutCaptureNorPawnMoved(
			int numberOfMovesWithoutCaptureNorPawnMoved) {
		this.numberOfMovesWithoutCaptureNorPawnMoved = numberOfMovesWithoutCaptureNorPawnMoved;
	}
	...
	public State copy() {
		return new State(turn, board, canCastleKingSide, canCastleQueenSide,
	...
	public int hashCode() {
		return Objects.hashCode(turn, Arrays.deepHashCode(board),
				Arrays.hashCode(canCastleKingSide),
				Arrays.hashCode(canCastleQueenSide), enpassantPosition,
	...
	public boolean equals(Object obj) {
		...
		return Arrays.deepEquals(board, other.board)
				...
				&& Objects.equal(numberOfMovesWithoutCaptureNorPawnMoved,
						other.numberOfMovesWithoutCaptureNorPawnMoved)
	...
	public String toString() {
		return "State ["
				...
				+ ", canCastleKingSide="
				+ Arrays.toString(canCastleKingSide)
				+ ", canCastleQueenSide="
				+ Arrays.toString(canCastleQueenSide)
				+ ", "
				+ (enpassantPosition != null ? "enpassantPosition="
						+ enpassantPosition + ", " : "")
